...
| C-09 | 敌人元素状态感知 | 感知敌人当前附着元素（视觉信号） | ❌ 缺失 |
| C-10 | 反应物交互 | 主动触发草原核（雷/火）、结晶（岩）等 | ❌ 缺失 |
| C-11 | 元素盾破坏 | 识别敌人元素盾类型，使用克制元素破盾 | ❌ 缺失 |
...
| C-14 | 大招无敌帧利用 | 利用 Q 技能无敌帧规避大伤害 | ❌ 缺失 |
| C-15 | 体力管理（战斗中） | 冲刺闪避的体力预算管理 | ❌ 缺失 |
| C-16 | 食物使用（战斗中） | 战斗中使用治疗/复活/增益食物 | ❌ 缺失 |
| C-17 | 角色死亡处理 | 角色死亡时切换到存活角色，使用复活食物 | ❌ 缺失 |
| C-18 | 全灭恢复 | 全队阵亡后的复活/重新挑战流程 | ❌ 缺失 |

### 7.4 Boss 战

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| C-19 | Boss 攻击模式识别 | 识别 Boss 蓄力动画，预判攻击类型 | ❌ 缺失 |
| C-20 | Boss 阶段转换处理 | 检测阶段转换，在无敌期停止输出保存技能 | ❌ 缺失 |
| C-21 | Boss 弱点攻击 | 在弱点暴露时集中攻击（如遗迹守卫核心） | ❌ 缺失 |
| C-22 | Boss 机制应对 | 处理特殊机制（如女士的寒冰/烈焰花朵，散兵的元素核心） | ❌ 缺失 |
| C-23 | 周本 Boss 策略 | 针对不同周本 Boss 的专门战术 | ❌ 缺失 |
| C-24 | 世界 Boss 轮刷 | 自动传送到 Boss 位置、战斗、消耗树脂领奖、循环 | ⚠️ 部分有 |
...
| C-32 | 战败分析 | 分析失败原因（输出不足/生存困难/机制错误） | ❌ 缺失 |
| C-33 | 放弃判断 | 判断当前战斗是否无法胜利，选择撤退 | ❌ 缺失 |
| C-34 | 极限发挥 | 在可赢的困难战斗中优化操作到极限 | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| R-01 | 升级材料需求计算 | 计算从当前等级到目标等级所需的经验书和摩拉 | ❌ 缺失 |
| R-02 | 突破材料需求计算 | 计算突破所需的全部材料清单 | ❌ 缺失 |
| R-03 | 材料获取路径规划 | 规划获取缺失材料的最优路径（Boss/秘境/采集） | ❌ 缺失 |
| R-04 | 自动角色升级 | 执行完整的角色升级 UI 操作流程 | ❌ 缺失 |
| R-05 | 自动角色突破 | 执行完整的角色突破 UI 操作流程 | ❌ 缺失 |
| R-06 | 养成优先级排序 | 根据队伍需求排列角色养成优先级 | ❌ 缺失 |

### 8.2 天赋升级

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| R-07 | 天赋书日程管理 | 知道今天是哪些天赋书可刷的日子 | ❌ 缺失 |
| R-08 | 天赋升级优先级 | 知道每个角色的哪个天赋优先升（通常是 Q > E > 平A） | ❌ 缺失 |
| R-09 | 天赋材料需求计算 | 计算天赋升级所需的书本/Boss材料/敌人掉落 | ❌ 缺失 |
| R-10 | 自动天赋升级 | 执行完整的天赋升级 UI 操作流程 | ❌ 缺失 |
| R-11 | 周本材料转换 | 使用梦之溶剂转换周本材料为所需类型 | ❌ 缺失 |

### 8.3 武器管理

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| R-12 | 武器选择推荐 | 根据角色推荐最佳武器（考虑稀有度和可用性） | ❌ 缺失 |
| R-13 | 武器升级 | 执行武器强化 UI 操作流程 | ❌ 缺失 |
| R-14 | 武器突破 | 执行武器突破 UI 操作流程 | ❌ 缺失 |
| R-15 | 武器精炼 | 执行武器精炼 UI 操作流程（消耗重复武器） | ❌ 缺失 |
| R-16 | 武器材料日程 | 武器突破秘境的日程管理 | ❌ 缺失 |
| R-17 | 锻造武器 | 在铁匠处锻造武器/强化矿 | ❌ 缺失 |

### 8.4 圣遗物管理

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| R-18 | 圣遗物套装知识 | 知道每个角色适合什么圣遗物套装 | ❌ 缺失 |
| R-19 | 圣遗物主词条知识 | 知道每个角色需要的沙/杯/头主词条 | ❌ 缺失 |
| R-20 | 圣遗物副词条评估 | 评估圣遗物副词条质量（暴击率/暴击伤害/充能等） | ❌ 缺失 |
| R-21 | 圣遗物自动装备 | 为角色快速装备最佳可用圣遗物 | ❌ 缺失 |
| R-22 | 圣遗物自动强化 | 选择有价值圣遗物并强化到目标等级 | ❌ 缺失 |
| R-23 | 圣遗物回收/喂养 | 将垃圾圣遗物作为强化材料消耗 | ❌ 缺失 |
| R-24 | 圣遗物合成台 | 使用神秘供奉转化 3 个五星圣遗物为目标套装 | ❌ 缺失 |
| R-25 | 圣遗物域刷取 | 自动刷取指定圣遗物秘境 | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| E-01 | 传送点发现 | 在大世界中发现未激活的传送点 | ⚠️ 需 VLM |
| E-02 | 传送点激活 | 走近传送点并按 F 激活 | ⚠️ 部分有 |
| E-03 | 七天神像激活 | 找到并激活区域七天神像（揭示地图区域） | ⚠️ 部分有 |
| E-04 | 系统化传送点收集 | 按区域规划路径解锁所有传送点 | ❌ 缺失 |

### 9.2 宝箱与收集

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| E-05 | 宝箱发现 | 通过视觉/宝箱罗盘发现附近宝箱 | ❌ 缺失 |
| E-06 | 宝箱开启 | 走近宝箱按 F 开启，处理可能的敌人守护 | ❌ 缺失 |
| E-07 | 宝箱罗盘使用 | 装备并使用区域宝箱罗盘道具 | ❌ 缺失 |
| E-08 | 神瞳收集 | 发现并收集各区域的神瞳（风神瞳/岩神瞳等） | ❌ 缺失 |
| E-09 | 神瞳共鸣石使用 | 使用神瞳共鸣石道具定位附近神瞳 | ❌ 缺失 |
| E-10 | 材料采集 | 采集世界中的植物/矿石/动物材料 | ❌ 缺失 |
| E-11 | 区域探索率追踪 | 追踪各区域探索完成百分比 | ❌ 缺失 |

### 9.3 谜题解决

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| E-12 | 元素方尖碑激活 | 用正确元素攻击元素方尖碑 | ❌ 缺失 |
| E-13 | 火把谜题 | 解 Lights-Out 类火把谜题 | ❌ 缺失 |
| E-14 | 赛伊尔引导 | 引导赛伊尔回到赛伊尔庭院 | ❌ 缺失 |
| E-15 | 时间挑战 | 完成限时跑酷/收集/战斗挑战 | ❌ 缺失 |
| E-16 | 压力板谜题 | 踩压力板触发机关 | ❌ 缺失 |
| E-17 | 区域特殊谜题 | 各区域特有谜题机制（稻妻雷立方、须弥四叶印、枫丹水泡等） | ❌ 缺失 |
| E-18 | VLM 辅助解谜 | 使用 VLM 分析谜题状态并推理解决方案 | ❌ 缺失 |

### 9.4 区域探索

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| E-19 | 区域解锁推进 | 按序推进蒙德→璃月→稻妻→须弥→枫丹→纳塔 | ⚠️ 部分有 (quest chain) |
| E-20 | 系统化区域扫荡 | 按区域规划高效收集路线 | ❌ 缺失 |
| E-21 | 地下区域探索 | 导航层岩巨渊、须弥洞穴、枫丹水下等地下区域 | ❌ 缺失 |
| E-22 | 环境适应 | 适应不同区域环境（严寒、雷暴、沙漠沙暴） | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| M-01 | 摩拉预算管理 | 跟踪摩拉收支，确保升级需求不超支 | ❌ 缺失 |
| M-02 | 原石预算管理 | 跟踪原石收入，规划抽卡预算 | ❌ 缺失 |
| M-03 | 树脂管理 | 跟踪树脂恢复进度，确保不溢出 | ❌ 缺失 |
| M-04 | 树脂分配策略 | 根据当前 AR 和需求分配树脂到不同活动 | ❌ 缺失 |
...
| M-09 | 材料缺口分析 | 对比目标养成计划和当前库存，列出缺口 | ❌ 缺失 |
| M-10 | 材料获取计划 | 根据缺口生成材料获取任务列表（刷哪个Boss/秘境） | ❌ 缺失 |

### 10.3 食物与消耗品

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| M-11 | 食物烹饪 | 在烹饪台制作食物 | ❌ 缺失 |
| M-12 | 食物库存管理 | 跟踪食物库存，确保关键食物充足（复活/治疗/增益） | ❌ 缺失 |
| M-13 | 战斗前食物准备 | 在困难战斗前使用增益食物（ATK/暴击/防御） | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| W-01 | 保底计数器 | 跟踪当前卡池的抽卡数和保底状态 | ❌ 缺失 |
| W-02 | 卡池分析 | 分析当前卡池角色/武器的价值 | ❌ 缺失 |
| W-03 | 抽卡执行 | 自动执行抽卡 UI 操作 | ❌ 缺失 |
| W-04 | 抽卡结果处理 | 识别抽卡结果，更新库存 | ❌ 缺失 |
| W-05 | 派蒙商店月购 | 每月购买纠缠之缘（5 个，750 星尘）和相遇之缘（5 个，375 星尘） | ❌ 缺失 |
| W-06 | 星辉角色购买 | 用星辉购买轮换 4 星角色（每月 2 个，各 34 星辉） | ❌ 缺失 |
| W-07 | 纪念品商店购买 | 用元素之印购买角色突破材料和武器蓝图 | ❌ 缺失 |
| W-08 | 抽卡策略决策 | 决定是否抽当前卡池还是攒原石等未来卡池 | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| S-01 | 当前阶段识别 | 判断当前处于早期(AR1-30)/中期(AR30-45)/后期(AR45+) | ⚠️ 部分有 |
| S-02 | 阶段优先级切换 | 根据阶段调整行动优先级 | ❌ 缺失 |
| S-03 | 瓶颈识别 | 识别当前阻碍进展的瓶颈（等级/装备/材料/任务） | ❌ 缺失 |
...
| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| L-01 | 战斗经验积累 | 记录每次战斗的经过和结果，积累经验 | ⚠️ 部分有 (BAGEL belief system) |
| L-02 | Boss 模式学习 | 从多次 Boss 战中总结攻击模式和弱点窗口 | ❌ 缺失 |
| L-03 | 战斗计划迭代 | 每次战斗失败后优化下一次的战斗 playbook | ❌ 缺失 |
| L-04 | 失败归因分析 | 区分"角色太弱"vs"操作不当"vs"不了解机制" | ❌ 缺失 |
| L-05 | 养成效果评估 | 评估升级/换装后的战斗力变化 | ❌ 缺失 |
| L-06 | 策略自适应 | 根据长期表现自动调整策略参数 | ❌ 缺失 |
| L-07 | 跨 Boss 知识迁移 | 从一个 Boss 学到的模式迁移到类似 Boss | ❌ 缺失 |
| L-08 | 在线攻略利用 | 搜索并利用人类玩家的攻略和经验 | ❌ 缺失 |
...
| 对话系统 (Dialog) | 9 | 3 | 4 | 2 |
| 任务系统 (Quest) | 16 | 4 | 3 | 9 |
| 战斗智能 (Combat) | 34 | 12 | 3 | 19 |
| 角色养成 (Progression) | 30 | 0 | 1 | 29 |
| 探索收集 (Exploration) | 22 | 0 | 3 | 19 |
| 资源管理 (Resource) | 16 | 0 | 0 | 16 |
| 抽卡商店 (Wish/Shop) | 8 | 0 | 0 | 8 |
| 日常循环 (Daily) | 7 | 0 | 0 | 7 |
| 战略大脑 (Strategy) | 16 | 1 | 2 | 13 |
| 元学习 (Meta-Learning) | 8 | 0 | 1 | 7 |
| **总计** | **274** | **48** | **37** | **189** |

**完成率：17.5%（48/274）| 部分完成：13.5%（37/274）| 需新建：69.0%（189/274）**

---

> **优先级建议**：首先建设 **UI 菜单导航宏库（U-23~U-44）**——这是连接感知与行动的桥梁，几乎所有高阶能力都依赖它。其次是 **角色养成自动化（第八节）** 和 **探索引擎（第九节）**，这两个是推进剧情的必要支撑。战斗智能已较为完善，重点补齐 Boss 机制学习和失败恢复即可。
