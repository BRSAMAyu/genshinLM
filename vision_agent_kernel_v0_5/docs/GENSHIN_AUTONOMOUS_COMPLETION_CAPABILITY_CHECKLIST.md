...
| P-10 | Boss 阶段转换检测 | 检测 Boss 进入无敌/新阶段的视觉信号（发光、形态变化） | ❌ 缺失 |
| P-11 | AoE 地面指示器检测 | 检测红色/橙色地面圆圈（即将到来的范围攻击） | ⚠️ 部分有 (danger_detector.py) |
| P-12 | 元素附着可视化检测 | 检测敌人身上的元素附着状态（火光环、冰蓝色等） | ❌ 缺失 |
| P-13 | 草原核/反应物检测 | 检测地面上的草原核、燃烧区域等可交互反应物 | ❌ 缺失 |
| P-14 | 天气/环境效果检测 | 检测严寒（龙脊雪山）、雷暴（稻妻）等环境计量条 | ❌ 缺失 |
| P-15 | 体力条检测 | 检测角色体力条当前值（攀爬/游泳/滑翔时关键） | ❌ 缺失 |
| P-16 | 倒计时/计时器检测 | 检测限时挑战的倒计时 UI 元素 | ❌ 缺失 |
| P-17 | 宝箱品质识别 | 区分 普通/精致/珍贵/华丽/奇珍 宝箱 | ❌ 缺失 |
...
| Q-05 | 任务日志管理 | 在任务菜单中浏览、追踪、切换任务 | ❌ 缺失 |
| Q-06 | 世界任务发现 | 发现并接取世界任务（NPC 黄色感叹号） | ❌ 缺失 |
| Q-07 | 委托任务识别 | 识别每日委托的类型和位置 | ❌ 缺失 |
| Q-08 | 任务完成确认 | 检测任务完成通知和奖励弹窗 | ⚠️ 部分有 |
| Q-09 | 任务占用处理 | 处理"NPC 被其他任务占用"的情况 | ❌ 缺失 |

### 6.2 特殊任务机制

| # | 能力 | 描述 | 状态 |
|---|------|------|------|
| Q-10 | 潜行任务 | 蒙德璃月千岩军潜行、稻妻离岛逃脱等潜行场景 | ❌ 缺失 |
| Q-11 | 护送任务 | 保护 NPC 移动到目标地点 | ❌ 缺失 |
| Q-12 | 限时挑战 | 在时间限制内完成战斗/收集 | ❌ 缺失 |
| Q-13 | 调查任务 | 枫丹梅罗彼得堡调查（线索收集、审讯） | ❌ 缺失 |
| Q-14 | 梦境序列 | 须弥梦境循环解谜 | ❌ 缺失 |
| Q-15 | 秘境任务执行 | 进入任务专属秘境，完成战斗+解谜 | ⚠️ 部分有 |
| Q-16 | AR 突破任务 | 完成 AR 25/35/45/50 的突破域 | ❌ 缺失 |
...
> **更新日期**：2026-05-30（Phase 1-8 实现后更新）

| 类别 | 总条目 | ✅ 已完成 | ⚠️ 部分有 | ❌ 缺失 |
|------|--------|----------|----------|---------|
| 感知层 (Perception) | 30 | 12 | 7 | 11 |
...
**完成率：45.6%（125/274）| 部分完成：19.3%（53/274）| 需新建：35.0%（96/274）**

---

> **当前优先级**：补齐 **感知层视觉检测**（P-10~P-18）和 **任务系统**（Q-05~Q-16）——这些是自主推进剧情的关键瓶颈。其次是 **UI 自动化执行**（将 ⚠️ 的 UI 流程模板升级为完整可执行流程）和 **导航特殊移动**（攀爬/游泳/滑翔）。
