...
# Map failure_code strings to FailureCategory enum values.
_FAILURE_CODE_MAP: dict[str, FailureCategory] = {
    ...
    "DANGER_UNAVOIDED": FailureCategory.DANGER_UNAVOIDED,
    "ELEMENT_MISMATCH": FailureCategory.ELEMENT_MISMATCH,
    "STAMINA_EXHAUSTED": FailureCategory.STAMINA_EXHAUSTED,
}


class GenshinFailureBridge:
    """Bridges Orchestrator failures to GenshinFailureAnalyzer."""

    def __init__(self, state_bus: StateBus) -> None:
        self._state_bus = state_bus
        self._analyzer = GenshinFailureAnalyzer()
        self._failure_slot = state_bus.register_slot("genshin.failure_state")

    def on_skill_result(self, result: SkillResult) -> None:
        """Called when a skill completes. Records failures and publishes analysis."""
        if result.status != "FAILED":
            return

        category = _FAILURE_CODE_MAP.get(
